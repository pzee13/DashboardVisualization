...
function App() {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get('http://localhost:5000/api/world-details')
            .then(response => {
    ...
    const totalSectors = new Set(allData.map(item => item.sector)).size;
    const totalCountries = new Set(allData.map(item => item.country)).size;
    const totalTopics = new Set(allData.map(item => item.topic)).size;

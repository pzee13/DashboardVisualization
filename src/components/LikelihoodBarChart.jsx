...
const LikelihoodBarChart = ({ data }) => {
    ...
    useEffect(() => {
        ...
        ).map(([sector, likelihood]) => ({ sector, likelihood }));
        ...
        const width = 600;
        const height = 400;
        const margin = { top: 20, right: 20, bottom: 50, left: 70 }; // Increased bottom margin
        ...
        const maxLikelihood = d3.max(processedData, d => d.likelihood);
        const y = d3.scaleLinear()
            .range([height - margin.bottom, margin.top])
            .domain([0, maxLikelihood * 1.1]);
